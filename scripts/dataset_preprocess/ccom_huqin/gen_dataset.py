import csv
import os
import pandas as pd
import librosa
import torch
import torchaudio
import numpy as np
from pydub import AudioSegment
from pathlib import Path

DATA_DIR = {"train": "ccom/train",
            "val": "ccom/val",
            "test": "ccom/test"}

EmptyToken = 'NA'
# label to index
# with open('index2class.txt') as f:
#     l2i = {x[1]: int(x[0])
#            for x in map(lambda x: x.strip().split(','), f)}

STRIDE = 5
WINDOW_SIZE = 10
TARGET_SR = 16000


def generate_dataset(audio_dir, save_meta):
    key = 0
    all_label_list = []

    if not os.path.exists(audio_dir):
        raise FileNotFoundError
    else:
        for fullpath in Path(audio_dir).glob('**/*.wav'):
            audio_path = str(fullpath)
            file_path = os.path.split(fullpath)[0]
            audio_file = os.path.split(fullpath)[1]
            save_path = file_path.replace('ccomhuqin_old', 'ccomhuqin')
            Path(save_path).mkdir(parents=True, exist_ok=True)
            all_label_list += split2segments(audio_path, os.path.join(save_path, audio_file))
            #csv_file = str(fullpath).replace(".wav", ".csv")
            # list_of_clips = split2segments(str(fullpath))
            # for clip in list_of_clips:
            #     start, end = clip["start"], clip["end"]
            #     clip_name = clip["filename"]
            #     label_timelist = clip["label_timelist"]
            #     # 保存切好的片段audio
            #     wav = AudioSegment.from_file(fullpath, start_second=start, duration=end-start)
            #     converted_wav = wav.set_frame_rate(TARGET_SR).split_to_mono()
            #     converted_wav.export(os.path.join(save_path, clip_name), format="wav")
            #     # 保存片段对应的label
            #     all_label_list.append(label_timelist)
    # 所有的片段对应的label存成一个tsv
    with open(save_meta, "w") as f_output:
        tsv_output = csv.writer(f_output, delimiter='\t')
        tsv_output.writerow(["filename", "onset", "offset", "event_label"])
        tsv_output.writerows(all_label_list)

def split2segments(audio_path, save_path):
    csv_path = audio_path.replace(".wav", ".csv")
    annotation = pd.read_csv(csv_path)
    audio_len = librosa.get_duration(filename=audio_path)
    waveform, sr = torchaudio.load(audio_path, normalize=True)
    waveform = torchaudio.functional.resample(waveform, sr, TARGET_SR)
    waveform_mono = torch.mean(waveform, dim=0, keepdim=True)  # 双声道/立体声转成单声道
    # length = waveform_mono.shape[-1]
    # seg_length = int(self.sr) * self.seg_len
    print("my_dataset", audio_path)

    intresserad = dict()
    for (onset, duration, label) in zip(
            *map(annotation.get, ['onset', 'duration', 'PT1-1'])):
        if not pd.isnull(label):
            intresserad[(onset, duration)] = label

    upper = max(int(audio_len) - (STRIDE - 1), 1)
    # print("upper: ", upper)
    clip_list = list()
    label_timelist = [] # label_timelist是记录10s片段当中的label对应的onset和offset
    for curr in range(0, upper, STRIDE):
        start, end = curr, curr + WINDOW_SIZE
        new_filename = save_path.replace(".wav", f"_{start}_{end}.wav")
        # 保存切好的片段audio
        start_idx, end_idx = int(start*TARGET_SR), int(end*TARGET_SR)
        clip = waveform_mono[:, start_idx: end_idx]
        torchaudio.save(new_filename, clip, TARGET_SR)
        for onset, duration in intresserad:
            label = intresserad[(onset, duration)]
            # 标记的PT有任意一段在当前5s clip内，都要标记对应的frame
            offset = onset + duration
            if offset <= start or end <= onset:
                # no overlap
                continue
            elif onset <= start <= offset:
                onset = start
            elif onset <= end <= offset:
                offset = end
            elif onset <= start <= end <= offset:
                onset, offset = start, end

            new_onset, new_offset = onset - start, offset - start
            label_timelist.append([new_filename, new_onset, new_offset, label])

            # convert to relative timing
            # frame_onset, frame_offset = map(lambda x: librosa.time_to_frames(x, sr=sr, hop_length=320),
            #                                 [new_onset, new_offset])
            # labels[frame_onset: frame_offset] = l2i[label]

        # clip_list.append({
        #     "start": start,
        #     "end": end,
        #     #"labels": labels,
        #     "label_timelist": label_timelist,
        #     "filename": new_filename
        #     #"file": audio_path
        # })

    return label_timelist

if __name__ == "__main__":
    # generate_dataset(audio_dir="/root/autodl-tmp/ccomhuqin_old/data/train",
    #                  save_meta="/root/autodl-tmp/ccomhuqin/meta/train/train.tsv")
    generate_dataset(audio_dir="/root/autodl-tmp/ccomhuqin_old/data/val",
                     save_meta="/root/autodl-tmp/ccomhuqin/meta/val/val.tsv")
    # generate_dataset(audio_dir="/root/autodl-tmp/ccomhuqin_old/data/test",
    #                  save_meta="/root/autodl-tmp/ccomhuqin/meta/test/eval.tsv")
