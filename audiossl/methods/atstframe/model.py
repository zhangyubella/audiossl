    ...
    def __init__(self,
        ...
        if avg_blocks == 0:  # atst-frame
            ...
            self.teacher = MultiCropWrapper(
                encoder_fn(pos_type=pos_type, patch_embed=patch_embed, avg_blocks=8, **kwargs),
                embed_dim,
                projector=None,
                predictor=False)
...
class FrameATSTLightningModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("total_loss_frm", total_loss_frm, on_step=False, on_epoch=True, logger=True)
        self.log("std_frm_tea", std_frm_tea, logger=True)
        self.log("std_frm_stu", std_frm_stu, logger=True)
        self.log("ema", self.ema_scheduler[self.global_step], logger=True)
        self.log("step", float(self.global_step), on_epoch=True, logger=True)
    ...
    def validation_step(self, batch, batch_idx):
        (melspecs, lengths, masks), _ = batch
        total_loss_frm, std_frm_stu, std_frm_tea = self.model(melspecs, lengths, masks)
        self.log("validation_loss", total_loss_frm, on_step=False, on_epoch=True, logger=True)
    ...
    def schedule(self):
        ...
        self.log("lr", param_group["lr"], prog_bar=True, on_epoch=True, logger=True)

    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(get_params_groups(self.model.student),
