...
class FrameATSTTrainTransform:
    def __init__(self,sr=16000,n_fft=1024, win_length=1024,hop_length=1024, aug_tea=True,aug_stu=True,freq_wrap=True,
                 mask_ratio=0.75,mask_nooverlap=False,min_mask_len=2,mask_len=5,mask_type="random",
                 anchor_len=6.,patch_h=64,patch_w=4,n_mels=64,**kwargs):
        melspec_t = torchaudio.transforms.MelSpectrogram(
            sample_rate=sr, f_min=60, f_max=7800, hop_length=hop_length, win_length=win_length, n_fft=n_fft, n_mels=n_mels)
    ...
    def __call__(self,input):
        ...
        self.positivecrop.transforms[0].size=int(anchor_len*self.sr)
        ...
        num_patches = get_num_patches(self.n_mels,int(anchor_len*self.sr)//self.hop_length + 1,self.patch_h,self.patch_w)
        ...
        crops.append(F.pad(self.positive_transform1(crop_positive1),
                          (0,int((self.max_positive_len*self.sr)//self.hop_length-int(anchor_len*self.sr)//self.hop_length))))
        lengths.append(int(anchor_len*self.sr)//self.hop_length+1)
        crops.append(F.pad(self.positive_transform2(crop_positive2),
                          (0,int((self.max_positive_len*self.sr)//self.hop_length-int(positive_len*self.sr)//self.hop_length))))
        lengths.append(int(positive_len*self.sr)//self.hop_length+1)
        masks.extend([mask]*2)
